...
.controller('RegisterCtrl', function($scope, $location, $cookieStore){
  $scope.user = {};
  ...
  $scope.moveNext = function(){
    $location.url('/contact/' + $scope.user.email);
  }
})

.controller('ContactCtrl', function($scope, $location, $http, $stateParams, $cookieStore){
  $scope.user = {
    user: $stateParams.email,
    nums: []
  }
  ...
  $scope.register = function() {
    ...
    $http({
      method: 'POST',
      url: 'http://tasuke.mybluemix.net/register.php',
...
.controller('HomeCtrl', function($scope, $rootScope, $ionicPush, $ionicUser, $cordovaGeolocation, $http, $cookieStore) {
  var posOptions = {
    timeout: 10000,
  ...
  var getLocationName = function(lat, long) {
    $http.get('http://tasuke.mybluemix.net/country.php?lat=' + lat + '&long=' + long).
    success(function(data, status, headers, config) {
      $scope.location = data.geonames[0].countryName + ', ' + data.geonames[0].name;

      countryInfo = data;
    });
  }

  $scope.calling = function(type) {
    window.open('tel:' + countryInfo.info[type]);

    $http.get('http://mimiko.me/Tasuke/call.php?lat=' + location.lat + '&long=' + location.long + '&type=' + type + '&email=' + user).
    success(function(data, status, headers, config) {
      console.log(data);
    })
  }
